...
const handleFallback = (stores: any[]) => {
  ...
  return {
    store: mainStore,
    message:
      "Menggunakan toko utama (Lokasi di luar jangkauan atau belum terdeteksi)",
  };
};

export const getAssignedStore = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  try {
    ...
    if (!address || address.latitude === null || address.longitude === null) {
      return sendResponse(
        ...
        404,
        false,
        "Address not found or location not set",
      );
    }
...
export const getFallbackStore = async (
  _req: Request,
  res: Response,
  next: NextFunction,
) => {
...
export const getNearestStore = async (
  ...
  try {
    ...
    if (!stores.length)
      return sendResponse(res, 404, false, "Data toko kosong");
...
export const assignStoreAdmin = async (
  ...
  try {
    ...
    return sendResponse(
      ...
      200,
      true,
      "Admin berhasil ditugaskan ke cabang ini",
    );
  } catch (error) {
    next(error);
  }
