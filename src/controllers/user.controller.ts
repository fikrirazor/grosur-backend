    ...
    if (req.body && req.body.referralCode) {
      ...
      if (newRefCode !== (req as any).user.referralCode) {
        ...
        if (existingRef && existingRef.id !== userId) {
          return res.status(400).json({ message: "Kode referral tersebut sudah dipakai pengguna lain" });
        }
...
const executeProfileUpdate = async (id: string, data: any) => {
  return await prisma.user.update({
    where: { id },
    data,
    select: { id: true, email: true, name: true, phone: true, role: true, photo: true, referralCode: true }
  });
};
...
export const updatePassword = async (req: Request, res: Response) => {
  try {
    ...
    if (!user || !user.password) {
      return res.status(400).json({ message: "User tidak ditemukan atau belum mengatur password" });
    }
